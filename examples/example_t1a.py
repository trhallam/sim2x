...
# %%
from sim2x.datasets import fetch_example_data

# %%
t1a = fetch_example_data()["t1a.zip"]
...
imps = sim2imp(
    ...
    {"PRESSURE":"pres", "SWAT":"swat", "PORO":"poro"}, # simulator props in sim.data mapped to rock model props
...
arb_line = arb_line.set_coords(("cdp_x", "cdp_y")) # make coordinates so they get inheritted by child volumes (e.g. gi_vol)
...
gi_vol = cpgrid_to_rg(arb_line.drop("twt"), sim.xyzcorn, mapping_dims=("cdp", ), srate=1, buffer=50)
display(gi_vol)
gi_vol.gi.T.plot(yincrease=False, vmin=-1, vmax=75)

# %%
gi_vol3d = cpgrid_to_rg(vol_3d.drop("twt"), sim.xyzcorn, mapping_dims=("xline", "iline"), srate=1, buffer=50)
display(gi_vol3d)
...
report_rgs = {
    key:map_to_gi_vol(val, gi_vol) for key, val in imps.items()
}

# %%
report_rgs3d = {
    key:map_to_gi_vol(val, gi_vol3d) for key, val in imps.items()
}
...
# %%
for key, fill_value in [("vp", 3000), ("vs", 1550), ("density", 2.2), ("bulk_modulus", 14.5), ("shear_modulus", 5)]:
...
# %%
depth_samples = xr.DataArray(np.linspace(2430, 2490, 25), dims="cdp", coords={"cdp":twt_int.cdp}, name="depth")
dsx_, dsy_ = np.meshgrid(np.linspace(2430, 2490, 25), np.linspace(2430, 2490, 25))
dsm_ = (dsx_ + dsy_) / 2
depth_samples3d = xr.DataArray(dsm_, dims=("iline", "xline"), coords={"iline":time_int3d.iline, "xline":time_int3d.xline}, name="depth")
...
peg_twt2500 = twt_conv.peg_time_interval(twt_int, depth_samples, twt_2500mps, mapping_dims=("cdp", ))

# converting to twt
rem_twt = twt_conv.domain_convert_vol(peg_twt2500, rem, mapping_dims=("cdp",), output_samples=np.arange(1.2, 1.27, 0.001))
...
# %%
rem_twt_z = twt_conv.domain_convert_vol(peg_twt2500, rem_twt, mapping_dims=("cdp",), domain_vol_direction="reverse", from_dim="twt", to_dim="depth")
...
twt_conv.time_convert_surface(peg_twt2500, depth_samples, mapping_dims=("cdp", )).plot(ax=axs)
twt_2500mps.plot(ax=axs)


# %%
peg3d_z2500 = twt_conv.domain_convert_vol(peg3d_twt2500, xr.Dataset({"z":peg3d_twt2500.depth.broadcast_like(peg3d_twt2500)}))

# %%
peg3d_z2500.sel(iline=10).z.plot()

# %%
peg3d_z2500_rtwt = twt_conv.domain_convert_vol(peg3d_twt2500, peg3d_z2500, from_dim="twt", to_dim="depth", domain_vol_direction="reverse")
...
# %%
a = seis_synth.reflectivity(5, rem_twt.sel(cdp=10).vp.values, rem_twt.sel(cdp=10).vs.values, rem_twt.sel(cdp=10).density.values, method="zoep_pud")
...
# %%
seis_depth = twt_conv.domain_convert_vol(peg_twt2500, seis, from_dim="twt", to_dim="depth", domain_vol_direction="reverse", mapping_dims=("cdp", ), interpolation_kind="linear")
seis_depth_cubic = twt_conv.domain_convert_vol(peg_twt2500, seis, from_dim="twt", to_dim="depth", domain_vol_direction="reverse", mapping_dims=("cdp", ), interpolation_kind="cubic")
seis_depth_sinc = twt_conv.domain_convert_vol(peg_twt2500, seis, from_dim="twt", to_dim="depth", domain_vol_direction="reverse", mapping_dims=("cdp", ), interpolation_kind="sinc")
